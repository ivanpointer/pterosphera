...
module switch_col_arc(count, home_row, radius) {
    ...
    if ($pdebug) color("black") sphere(1);
    ...
    // Work out the trig bits for the arc
    ...
    // Render the arc for the switches
    polyhedron(points = points, faces = faces);



    // Extend the body walls
    switch_clearance_R = col_R2 + switch_socket_clearance;

    // Bottom inner switch face (bisf...)
    bisfp1 = [ _arc_of_points_cos_n(0,a,ao,switch_clearance_R), mxy[0], _arc_of_points_sin_n(0,a,ao,switch_clearance_R) ];
    bisfp2 = [ bisfp1.x, mxy[1], bisfp1.z ];
    bisf_p = [ points[count + 1], points[(count+1)*3], bisfp1, bisfp2 ];
    bisf = [ [0, 1, 2, 3] ];
    echo(bisf_p);
    echo(bisf);
    polyhedron(points = bisf_p, faces = bisf);
}
